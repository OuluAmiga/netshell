...
int main(int argc, char *argv[])
{
    ...
    if (app) {
        ...
        {   /* Event loop */
            ...
            // Wait for application messages or Ctrl+C
            while (!done) {
                // Use MUI_EventHandlerBase to get the app's signal bit
                ULONG appSig = 1L << MUI_EventHandlerBase(app);
                
                Wait(appSig | SIGBREAKF_CTRL_C);
                
                while (MUI_ApplicationGet(app, &class, &code)) {
